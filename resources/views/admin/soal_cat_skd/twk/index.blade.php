                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data.code == '200') {
                                    ...
                                    setTimeout(function() {
                                        window.location =
                                        "{{ action('SoalCatSkdController@twk') }}";
                                    }, 1500);
...
@section('button-title')
    <a class="btn btn-sm btn-primary  ml-2 float-right" href="{{ action('SoalCatSkdController@createTwk') }}"
                    ...
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ action('SoalCatSkdController@editTwk', $item->id) }}"
                                    class="btn btn-xs btn-warning">Edit</a>
                                <button class="btn btn-xs btn-danger hapus" type="button"
                                    data-url="{{ action('SoalCatSkdController@deleteTwk', $item->id) }}">Hapus</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>

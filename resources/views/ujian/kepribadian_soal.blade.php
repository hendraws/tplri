<div class="row mt-5" id="content-soal">
    <ol>
        ...
        @endforeach
        @php
            $angka = $soalSesi1->count();
        @endphp
        @foreach ($soalSesi2 as $soal )
        @php
            $angka++ ;
        @endphp
        <div style="display:none;" id="list-{{ $angka }}" class="list-soal">
            <h5>Soal</h5>
            <div class="col-12">
                <li value="{{ $angka }}"> {!! $soal->pertanyaan !!} </li>
            </div>
            <div class="col-12">
                <ol type='A' class='p-0'>
            ...
            <div class="col-12" >
                <a href="javascript:void(0)" class="btn btn-primary simpan" data-ujian="{{ $ujian->id }}" data-soal="{{ $soal->id }}" data-urutan="{{ $angka }}" data-sesi={{ $soal->sesi }} >Simpan</a>
                <a href="javascript:void(0)" class="btn btn-primary lanjutkan" data-ujian="{{ $ujian->id }}" data-soal="{{ $soal->id }}" data-urutan="{{ $angka }}" data-sesi={{ $soal->sesi }} >Lanjutkan</a>
            </div>
        </div>
        @endforeach
